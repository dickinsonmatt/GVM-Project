...
print(Q1_plot_100_cat)
```

```{r Question 1 100% by age}
Q1_plot_100_age <- Q1_data |>
...
Q7_data <- gvm_data |>
  ...
  add_row(
    ...
    num_articles = c(9),
    count = c(0),
    number_in_age_group = rep(age_group_nums$number_in_age_group[3], 1),
    num_in_category = c(1)) |>
  add_row(
    age = c("66+"),
    num_articles = c(9),
    count = c(0),
    number_in_age_group = rep(age_group_nums$number_in_age_group[5], 1),
    num_in_category = c(1)) |>
  mutate(
    age_percent = (count/number_in_age_group)*100,
...
```{r Question 7 plot}
Q7_plot <- Q7_data |> 
  ggplot(aes(as.factor(num_articles), articles_percent, fill = age)) +
  geom_col(position = "dodge", color = "black", linewidth = .1) +
  scale_fill_viridis_d(direction = -1) +
  scale_y_continuous(
...
# put in zeros for other age groups
```
