...
#if we want to compare to the past survey, do the same categries as are in 2021
...
website_likert_df_m <- website_likert_df_m %>%
  mutate(Item= case_when(
    ...
    Item == 'website_design_likert' ~ 'Design & Visual Elements \naid in Understanding & Enjoyment',
    Item == 'website_connected_likert' ~ 'I feel more connected to \nGVSU after reading GVM website',
    Item == 'website_proud_likert' ~ 'Reading GVM website makes me \nproud to be part of \nthe GVSU community',
    TRUE ~ as.character(Item)
  ))

#facet by question with rows of ages
q4 <- website_likert_df_m %>% ggplot()+
  geom_bar(aes(x = fct_relevel(Group,"66+","50-65","36-49","25-35","17-24"), y=value, fill = fct_relevel(variable,"Disagree","Neutral","Agree")), 
  ...
  scale_fill_manual(
    values = c(Disagree = "orange", Neutral = "grey", Agree = "skyblue")) +
  geom_label(aes(x = Group, 
                 y = value, 
                 label = ifelse(variable !="Neutral",as.character(paste(round(value),"%",sep="")), NA)),
             positon = position_stack(vjust = .5, reverse = TRUE),
             hjust = .5)
...
ggsave(filename = 'Q4_plot.png', width = 10, height = 7, path = "C:/Users/foste/Documents/GVM-Project/plots")
...
print_likert_df_m <- print_likert_df_m %>%
  mutate(Item= case_when(
    ...
    Item == 'print_proud_likert' ~ 'Reading print issues of Grand \nValley Magazine makes me proud \nto be part of the GVSU community.',
    TRUE ~ as.character(Item)
  ))



#filtering labels for percentage valyes >10
#filtered_df <- print_likert_df_m[print_likert_df_m$value >= 10,]

#facet by question with rows of ages
q5 <- print_likert_df_m %>% ggplot()+
  geom_bar(aes(x = fct_relevel(Group,"66+","50-65","36-49","25-35","17-24"), y=value, fill = fct_relevel(variable,"Disagree","Neutral","Agree")), 
               position=position_stack(reverse = TRUE), stat="identity", color ="black") +
  facet_wrap(~Item,nrow = 1) +
  coord_flip() +
...
#saving plot to files
ggsave(filename = 'Q5_plot.png', width = 10, height = 7, path = "C:/Users/foste/Documents/GVM-Project/plots")
...
#defining a function to calculate row percentage of each age group
perc_func<- function(mydf,myvar) {
  mydf <- mydf %>%
  group_by(age, {{myvar}}) %>%
  summarize(Sum_Count = sum(Count)) %>%
  group_by(age) %>%
  mutate(percent = Sum_Count / sum(Sum_Count) * 100)
  return(mydf)
}

#calculate percentage (each age group sum percentage=100)
where_read_by_age <- perc_func(where_read_by_age,where_read)


#ggplot
agep_q2<-where_read_by_age %>% ggplot(aes(x=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"), y=percent, fill=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"))) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "Where do you read GVM?",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age) +
...
#saving plot to files
ggsave(filename = 'Q2_plot.png', path = "C:/Users/foste/Documents/GVM-Project/plots")
```

# Engagement bt Age
```{r Engagement by Age, warning=FALSE}
#Create data frame with the counts of each typ 
...
#calculate percentage (each age group sum percentage=100)
engagement_by_age <- perc_func(engagement_by_age,gvsu_engagement)


#ggplot
agep_q6 <- engagement_by_age %>% ggplot(aes(x=fct_relevel(gvsu_engagement,"Agree","Neutral","Disagree"), 
                                 y=percent, fill=fct_relevel(gvsu_engagement,"Agree","Neutral","Disagree"))) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "After Reading Are You More Likley to Engage in Events/Activities by Age?",
      y = "Percent",
      fill = "Response",
      x = "") +
  facet_grid(~age) +
...
#Sango suggestion for same question
#agep_q6_sango <- engagement_by_age %>% ggplot(aes(x=fct_relevel(gvsu_engagement,"Agree","Neutral","Disagree"), 
#                                 y=percent, fill=age)) +
#  geom_bar(stat = 'identity', color='black') +
##  coord_flip() +
#  theme(axis.title.y = element_blank()) +
# labs(fill = "Response",
#       title = "After Reading Are You More Likley to Engage in Events/Activities by Age? (Q6)")+
#  scale_fill_viridis_d(direction = -1)

#showing both plots
#grid.arrange(agep_q6,agep_q6_sango,ncol=2)

#saving plot to files
ggsave(filename = 'Q6_plot.png', path = "C:/Users/foste/Documents/GVM-Project/plots")
...
drive_to_website_by_age <- perc_func(drive_to_website_by_age,drive_to_website)

#ggplot
agep_q8 <- drive_to_website_by_age %>% ggplot(aes(x=drive_to_website, y=percent, fill=as.factor(drive_to_website))) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "Driver of Website Visitation by Age",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age) +
...
#saving plot to files
ggsave(filename = 'Q8_plot.png', path = "C:/Users/foste/Documents/GVM-Project/plots")
```
...
website_engagement_by_age <- perc_func(website_engagement_by_age,website_engagement)


#ggplot
agep_q10 <- website_engagement_by_age %>% ggplot(aes(x=website_engagement, y=percent, fill=website_engagement)) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
...
#saving plot to files
ggsave(filename = 'Q10_plot.png', path = "C:/Users/foste/Documents/GVM-Project/plots")
```
...
opting_by_age <- perc_func(opting_by_age,opting)

#ggplot
agep_q12 <- opting_by_age %>% ggplot(aes(x=opting, y=percent, fill=opting)) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
...
#saving plot to files
ggsave(filename = 'Q12_plot.png', path = "C:/Users/foste/Documents/GVM-Project/plots")
```

```{r printing all even age graphs}
grid.arrange(agep_q2,agep_q6,agep_q8,agep_q10,agep_q12,ncol=3)
```
