...
```{r website likert}
#filtering in responses of those who say they read the website
...
website_likert_df_m <- website_likert_df_m %>%
  mutate(variable= case_when(
    ...
    variable == '2' ~ 'Slighly Disagree',
    variable == '3' ~ 'Neutral',
    variable == '4' ~ 'Slighly Agree',
    variable == '5' ~ 'Strongly Agree',
    TRUE ~ as.character(variable)
  ))

#facet by age, with rows of questions
q4_1 <- website_likert_df_m %>% ggplot()+
  geom_bar(aes(x = Item, y=value, fill = fct_relevel(variable,"Strongly Disagree","Slighly Disagree","Neutral","Slighly Agree","Strongly Agree")),position=position_stack(reverse = TRUE), stat="identity", color ="black") +
  ...
  labs(fill = "Response",
       x = "Question",
       y = "Percent",
       title = "Website Likert by Age (filtered for those who've read >= 1 article)") +
...
q4_2 <- website_likert_df_m %>% ggplot()+
  geom_bar(aes(x = fct_relevel(Group,"66+","50-65","36-49","25-35","17-24"), y=value, fill = fct_relevel(variable,"Strongly Disagree","Slighly Disagree","Neutral","Slighly Agree","Strongly Agree")), 
...
#       x = "Question",
#       y = "Percent",
#       title = "Print Likert by Age (filtered for those who've read >= 1 issue)") +
#  theme(legend.position = "bottom",
#        axis.title.y = element_blank()) +
#  scale_fill_colorblind() +
#  theme(aspect.ratio = 3)
...
agep_q2<-where_read_by_age %>% ggplot(aes(x=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"), y=percent, fill=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"))) +
  ...
  labs(title = "Where do you read GVM? (Q2)",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age) +
...
agep_q6 <- engagement_by_age %>% ggplot(aes(x=fct_relevel(gvsu_engagement,"Agree","Neutral","Disagree"), 
  ...
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "After Reading Are You More Likley to Engage in Events/Activities by Age? (Q6)",
      y = "Percent",
      fill = "Response",
      x = "") +
  facet_grid(~age) +
...
#we decided the left was superior
...
agep_q10 <- website_engagement_by_age %>% ggplot(aes(x=website_engagement, y=percent, fill=website_engagement)) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "How to Make Website More Engaging by Age (Q10)",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age) +
...
agep_q12 <- opting_by_age %>% ggplot(aes(x=opting, y=percent, fill=opting)) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "Opting into Print Issues by Age (Q12)",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age) +
