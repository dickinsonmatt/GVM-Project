...
#filtering out responses of those who've read zero website articles
website_likert_df <- gvm_clean %>%
  filter(num_articles != 0)

#making df for website likert questions
website_likert_df <- select(website_likert_df, starts_with("website_") & ends_with("_likert"))

#making all variables factors
website_likert_df <- data.frame(lapply(website_likert_df, as.factor))

#plotting likert 
wl_format<- likert(website_likert_df)
wl_plot <- plot(wl_format)
