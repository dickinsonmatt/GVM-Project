...
gvm_clean <- gvm_clean |>
  mutate(
    key_pop = as.factor(case_when(
      (relation_alumni == 1 & age %in% c("36-49", "50-65", "66+")) ~ "Alumni over 30",
      TRUE ~ "Other")))
...
age_by_action <- gvm_clean |>
  ...
  mutate(action = str_replace_all(action, "action_", ""), # simple text cleaning
         ...
         percent = (n/nrow(gvm_clean))*100)

age_by_action |>
  ggplot(aes(fct_reorder(action, (percent)), percent, fill = age)) + # grab variables and reorder action
  geom_col()  +
  theme_bw() +
  ...
  labs(
    ...
    y = "Percent",
    fill = "Age group",
    title = "Actions taken by Age group (n = 133)"
  ) +
...
key_engage_read <- gvm_clean |>
  ...
  summarize(n = n(),
            percent = case_when(
              key_pop == "Alumni over 35" ~ n/nrow(gvm_clean |> filter(key_pop == "Alumni over 30")),
...
gvm_clean <- gvm_clean |>
  mutate(num_action = sum(action_saved_article, action_discussed_article, na.rm = TRUE))

key_web_engage <- gvm_clean |>
  group_by(key_pop, website_engagment) |>
  summarize(n = n(),
            percent = case_when(
              key_pop == "Alumni over 30" ~ n/nrow(gvm_clean |> filter(key_pop == "Alumni over 30")),
              TRUE ~ n/nrow(gvm_clean |> filter(key_pop == "Other")
            ))*100)
...
```{r engaging with GV more more actions}
engage_action <- gvm_clean |>
  select(gvsu_engagement, starts_with("action")) |> # grabbing the variables of interest
  pivot_longer(!gvsu_engagement, names_to = "action", values_to = "count") |> # pivoting
  group_by(gvsu_engagement, action) |> # grouping to make summarizing
  summarize(n = sum(count, na.rm = TRUE)) |> # getting the counts for each category
  mutate(action = str_replace_all(action, "action_", ""), # simple text cleaning
         action = str_replace_all(action, "_", " "),
         action = str_to_title(action))
...
engage_action |>
  ggplot(aes(gvsu_engagement, n, fill = action)) +
  geom_col() +
  theme_bw() +
  scale_fill_viridis_d() 
```
