...
agep_q2<-where_read_by_age %>% ggplot(aes(x=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"), y=percent, fill=fct_relevel(where_read,"GVM Website","Both","I don't read the GVM","Print issues"))) +
  ...
  labs(title = "How do you read GVM?",
       y = "Percent",
       fill = "Response",
       x="") +
  facet_grid(~age,
...
events/activities associated with GVSU",
      y = "Percent",
      fill = "Response",
      x = "") +
  facet_grid(~age,
...
#Commented out this section of code after finding out the filtering results in the violation of a chi-square asssumption for expected counts

#filtering by having read at least 1 article or 1 issue
#temp_df_we <- gvm_clean %>%
...
#making age label
#ages_we <- website_engagement_by_age %>%
#     group_by(age) %>%
#     summarise(total_count = sum(Sum_Count))

#making flextable
#ages_we_count <- flextable(ages_we) %>% 
#                    set_header_labels(
#                      age = "Age",
#                      total_count = "Number in Category"
#                    )

#ages_we <- c(paste0("17-24 (n=",ages_we$total_count[1],")"),
#             paste0("25-35 (n=",ages_we$total_count[2],")"),
#             paste0("36-49 (n=",ages_we$total_count[3],")"),
#             paste0("50-65 (n=",ages_we$total_count[4],")"),
#             paste0("66+ (n=",ages_we$total_count[5],")"))
#names(ages_we) <- c("17-24", "25-35", "36-49", "50-65", "66+")

 


#need to create an array of characters that contains a lebal for how many there are
...
agep_q10 <- website_engagement_by_age %>% ggplot(aes(x=website_engagement, y=percent, fill=website_engagement)) +
  geom_bar(stat = 'identity', position='dodge', color='black') +
  labs(title = "How can we make the Grand Valley Magazine WEBSITE more engaging?",
       y = "Percent",
       fill = "Response",
       x="",
       caption = "") +
